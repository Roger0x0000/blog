...
# 基于连续系统设计的峰值电流模式Buck变换器


本节主要介绍峰值电流模式Buck变换器的基本计算和SIMPLIS仿真。
...
<center>
<img src="/assets/blog_image/PLECS_Buck/峰值电流模式对比.jpg" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/峰值电流Buck等效电路.jpg" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/次谐波震荡示意图.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/次谐波震荡条件.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/次谐波震荡与斜率补偿.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/斜率补偿引入位置.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/斜率补偿Vc_pp.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/正斜率补偿方式.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/斜坡补偿波特图.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/峰值电流模式Buck.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/负载和增益的关系.png" >
...
已知Buck变换器电路参数：最低输入电压$V_{inMin}=8V$，正常输入电压$V_{inNormal}=12V$，最高输入电压$V_{inMax}=18V$，输出电压$V_{Out}=5V$，输出电流$I_{oMax}=1A$，最小输出负载$R=5Ω$，开关频率$f_s = 350kHz$，穿越频率为$f_c = 10kHz$，电感电流反馈增益$K_{iL} （或者 R_i） =(20Ω/100)= 0.2$，电感$L = 56uH$，输出电容$C = 704uF$，输出电容等效串联电阻$R_{C\_ESR}（或者 R_c） = 40mΩ$。


根据上面的参数，我们可以使用Python画出传递函数的伯德图。


<center>
<img src="/assets/blog_image/PLECS_Buck/Buck_PCM_OpenLoop_Python.png" >
<center> 峰值电流模式Buck变换器伯德图 </center>
</center>
...
def buck_bode(Vin_Input,Rload):
    s = ctrl.tf('s')
    # 电感参数
    L = 56 * math.pow(10,-6) #uH
    R_L = 0*math.pow(10,-3) #m
    # 电容参数
    Co = 704 * math.pow(10,-6) #uF
    Rc = 40*math.pow(10,-3) #m   capacitor ESR (Equivalent Series Resistance) in ohm
    #输入和输出电压
    Vin = Vin_Input
    Vout = 5.0
    #稳态占空比
    ...
    Gc_plant3 = (R/Ri)*(1+s*Rc*Co)/(1+s*(Rc+R)*Co)
...
<center>
<img src="/assets/blog_image/PLECS_Buck/Buck_PCM_CloseLoop_Type2.png" >
...
<center>
<img src="/assets/blog_image/PLECS_Buck/Buck_PCM_CloseLoop_SIMPLIS.png" >
<center> 峰值电流模式Buck变换器 </center>
</center>


<center>
<img src="/assets/blog_image/PLECS_Buck/Buck_PCM_CloseLoop_SIMPLIS_Bode.png" >
<center> 峰值电流模式Buck变换器伯德图 </center>
</center>


仿真结果：穿越频率为8.94kHz，增益裕度为25dB和相位裕度为60.9°。
...
<font color=blue>参考文档：TI,"Switch-mode power
converter compensation
made easy" TI Literature
Number SLUP340 </font>
